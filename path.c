...
{
	char *comando_ruta, *directorio, *path_copy = NULL, *path = getenv("PATH");
	struct stat buffer;
	...
	{
		snprintf(comando_ruta, 1024, "%s/%s", directorio, comando);
		if (stat(comando_ruta, &buffer) == 0 && (buffer.st_mode & S_IXUSR))
		{
			free(path_copy);
			return (comando_ruta);
		}
